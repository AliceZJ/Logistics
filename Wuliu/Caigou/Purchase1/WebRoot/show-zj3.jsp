...
订单发送<br>
	<form action="PurServlet?opt=getpurchasebyid" method="post">
		采购订单号：<input type="text" name="selid"></input> <input type="submit"></input>
	</form>
	<div>
	...
	<table border="1" width="700">
		...
		<c:forEach items="${l }" var="p">
			<tr>
				...
				<td>${p.pur_man}</td>
				<td>${p.pur_status}</td>
				<td><a href="PurServlet?opt=updatepurchase_status&pur_id=${p.pur_id }">发送</a> | <a href="PurServlet?opt=updatepurchase_status&pur_id=${p.pur_id }">取消发送</a></td>
			</tr>
		</c:forEach>
	</table>
</body>
