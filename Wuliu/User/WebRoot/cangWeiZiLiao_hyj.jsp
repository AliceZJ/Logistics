	...
	$(function() {
		$("input[type=button][name=b_del]").click(function() {
			if (confirm("确定删除？")) {
				...
				window.location.href="delinfo?opt=4&id="+t;
			}
		});
		...
		$("input[type=button][name=b_change]").click(function() {
			var tr = $(this).parent().parent();
			var t = tr.find("td").eq(1).text();
			window.location.href="changecangwei.jsp";
		});
	});
</script>
</head>
<body>
	...
	<form action="" method="post">
		仓库仓位编号：<input name="pid" type="text"> <br> <br>
		仓库仓位名称：<input name="pname" type="text">&nbsp; <input
			name="research_user_btn" type="button" onClick="" value="查询">
		<br> <br>
		<div align="right" style="background-color:#9CF">
			<input name="add_btn" type="button" value="新增" onClick="window.location.href='addinfo?opt=4'">  <input
		...
		<div>
			<center>
				...
				name="b_del" type="button" value="删除">
							...
							<td><c:out value="${i.s_id_hyj}"></c:out>
							</td>
							<td><c:out value="${i.s_name_hyj}"></c:out>
							</td>
							<td><c:out value="${i.regions.r_name_hyj}"></c:out>
							</td>
							<td><c:out value="${i.s_area}"></c:out>
							</td>
						</tr>
					</c:forEach>
				</table>
