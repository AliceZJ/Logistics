...
<script type="text/javascript">
	$(function() {
		$("input[type=button][name=rece]").click(function(e) {
			if (confirm("确定签收？")) {
				...
				$("#norece").removeAttr("disabled");
			}
		});
		$("input[type=button][name=norece]").click(function(e) {
			if (confirm("确定取消签收？")) {
				//没有实现的：要记录改行的主键值，然后去数据库里修改订单状态
				/*var tr = $(this).parent().parent();
				tr.remove();
...
<body>
	...
	<form action="" method="post">
		订单编号： <input name="cus_id" type="text"> <input
		...
		<div>
			<table cellspacing="0px" cellpadding="10px">
				<tr bgcolor="#6699CC">
					...
					<td>提货日期</td>
					<td>订单状态</td>
					<td style="border-right-width:0px">选择操作</td>
				</tr>
				<c:forEach items="${l}" var="bt">
					<tr>
						<td><a href="cus_detial_servlet?id=${bt.order_id}"
							target="right"><c:out value="${bt.order_id}"></c:out> </a>
						</td>

						<td><c:out value="${bt.customer.customer_name}"></c:out>
						</td>
						<td><c:out value="${bt.order_date}"></c:out>
						</td>
						<td><c:out value="${bt.order_status}"></c:out></td>
						<td style="border-right-width:0px"><input type="button"
							name="rece" id="rece" value="签收" /> <input type="button"
							name="norece" id="norece" value="取消签收" disabled="disabled" /></td>
					</tr>
